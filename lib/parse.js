    ...
    if (where && typeof where === 'object' && keys.length) {
        return `WHERE ${keys.map(key => {
            ...
            if (key === '$like' || key === '&like') {
                ...
                return Object.keys(value).map($in => {
                    if (!Array.isArray(value[$in])) {
                        throw new Error('where in clause must be an array');
                    }
                    return `${ident($in)} in (${value[$in].map(getValue).join(', ')})`;
                }).join(' AND ');
            } else if (key === '+in') {
                return Object.keys(value).map($in => {
                    if (!Array.isArray(value[$in])) {
...
const getValue = parse.getValue = obj => {
